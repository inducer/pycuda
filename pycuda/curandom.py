...
__global__ single(%(state_type)s *s, const int n, float *d)
{
  const int id = blockIdx.x*blockDim.x+threadIdx.x;
  if (id < n)
    d[id] = curand_mtgp32_single(&s[id]);
}

__global__ specific(%(state_type)s *s, const int n, float *d,
    unsigned char *index, unsigned char *nn)
{
  const int id = blockIdx.x*blockDim.x+threadIdx.x;
  if (id < n)
    d[id] = curand_mtgp32_single_specific(&s[id], index[id], nn[id]);
}

__global__ specific(%(state_type)s *s, const int n, unsigned int *d,
    unsigned char *index, unsigned char *nn)
{
  const int id = blockIdx.x*blockDim.x+threadIdx.x;
  if (id < n)
...
if get_curand_version() >= (4, 1, 0):
    class Mtgp32RandomNumberGenerator(_RandomNumberGeneratorBase):
        ...
        def __init__(self, seed_getter=None, offset=0):
            ...
            super(XORWOWRandomNumberGenerator, self).__init__(
                seed_getter, offset,
                'curandStateMtgp32', 'unsigned int', 32, mtgp32_random_source)
            self.single = self.module.get_function("single")
            self.single.prepare("PiP")
            self.single_specific = self.module.get_function("single_specific")
            self.single_specific.prepare("PiPPP")
            self.specific = self.module.get_function("single_specific")
            self.specific.prepare("PiPPP")
        ...
        def _kernels(self):
            return (
                    list(self.generators.itervalues())
                    + [self.single, self.single_specific, self.specific])
        ...
        def generators_per_block(self):
            ...
            return min(256,
                super(Mtgp32RandomNumberGenerator, self).generators_per_block())

        def fill_single(self, data, stream=None):
            self.single.prepared_async_call(
                    (self.block_count, 1), (self.generators_per_block, 1, 1), stream,
                    self.state, self.generators_per_block * self.block_count, data)

        def fill_single_specific(self, data, index, n, stream=None):
            self.single_specific.prepared_async_call(
                    (self.block_count, 1), (self.generators_per_block, 1, 1), stream,
                    self.state, self.generators_per_block * self.block_count, data, index, n)

        def fill_specific(self, data, index, n, stream=None):
            self.specific.prepared_async_call(
                    (self.block_count, 1), (self.generators_per_block, 1, 1), stream,
                    self.state, self.generators_per_block * self.block_count, data, index, n)
